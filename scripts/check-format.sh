...
format_check_dir=$(mktemp -d "${TMPDIR:-/tmp}/sen-format-check.XXXXXX")
cleanup() {
...
done < <(find src tests examples scripts conda.recipe fonts benchmarks -type f -name '*.mojo' -print0)
